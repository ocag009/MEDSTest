  ...
  if [ "$_result"  == "" ]
    then
      OUTPUT=$($az group create --name $1 \
        --location $2 \
        --tags currentStatus=Group_Created RANDOM=$RANDOM_NUMBER CONTACT=$AZURE_USER AppID=EDM_MEDS Environment=PROD Owner=someone BU=Engineering \
        -ojsonc)
...
function CreateStorageAccount(){
    ...
    if [ "$_storage"  == "" ]; then
      OUTPUT=$(az storage account create \
        ...
        --tags AppID=EDM_MEDS Environment=PROD Owner=someone BU=Engineering \
        --query name -otsv)
...
function CreateOpenTestDataTemplateSpec() {
  ...
  $az ts create --name $TNO_TEMPLATE_NAME --resource-group $AZURE_GROUP --location $AZURE_LOCATION --version "1.0" --tags $SPGI_RESOURCE_TAGS --template-file $DEPLOYMENT_BLOB_TEMPLATE -o none
...
function CreatePortal() {
  ...
  if [ "$_plan"  == "" ]
  then
    $az appservice plan create -g $2 -n ${1} --is-linux --location $3 --tags $SPGI_RESOURCE_TAGS -o none
  else
  ...
  then
    ...
    $az webapp create -g $2 -p ${1} -n ${1} --https-only true --assign-identity "$MANAGED_IDENTITY" -i ${4}.azurecr.io/portal:latest --tags $SPGI_RESOURCE_TAGS -o none
