...
resource blobDeploymentScript 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  properties: {
    ...
    scriptContent: '''
      ...
      az storage blob upload -f experiencelab.sh -c $AZURE_STORAGE_CONTAINER -n experiencelab.sh -t $SPGI_RESOURCE_TAGS
      ...
      az storage blob upload -f template-open-test-data.json -c $AZURE_STORAGE_CONTAINER -n template-open-test-data.json -t $SPGI_RESOURCE_TAGS
      ...
      az storage blob upload -f downloadTnoData.sh -c $AZURE_STORAGE_CONTAINER -n downloadTnoData.sh -t $SPGI_RESOURCE_TAGS
      ...
      az storage blob upload -f developer-portal.tar.gz -c $AZURE_STORAGE_CONTAINER -n developer-portal.tar.gz -t $SPGI_RESOURCE_TAGS
      ...
      az storage blob upload -f rest-scripts.zip -c $AZURE_STORAGE_CONTAINER -n rest-scripts.zip -t $SPGI_RESOURCE_TAGS
    '''
  }
}
